...
// 1Sekon : "I don't think this is really worked"
...
vec3 renderOverworldSky(vec3 horizonEdgeCol, vec3 horizonColor, vec3 zenithColor, vec3 viewDir) {
  ...
  // gradient 1  h^108
  // gradient 2  h^8 mix h^3
  float gradient1 = hsq*hsq*hsq*hsq *hsq;
  float gradient2 = 0.6*gradient1 + 0.2*hsq;
  gradient1 *= gradient1;

//  vec3 sky = mix(horizonColor, horizonEdgeCol, gradient1); <== 1Sekon:"Nothing is happening"
  vec3 sky = mix(zenithColor,horizonColor, gradient2);

  return sky;
}

// sunrise/sunset bloom
vec3 getSunBloom(vec3 viewDir, vec3 FOG_COLOR) {
  float factor = duskD(FOG_COLOR);
  factor *= factor;
  ...
  vec3 r = mix(mix(vec3(0.0, 0.0, 0.0), vec3(1.5,0.5,0.05), sunBloom), vec3(1.0,0.4,0.05)*2.0+0.2, smoothstep(0.9, 2.0, sunBloom));
...
vec3 renderEndSky(vec3 horizonCol, vec3 zenithCol, vec3 viewDir, float t) {
  ...
  sky += nxfEndSpiral(viewDir, t, horizonCol);
...
vec3 nlRenderSky(vec3 horizonEdgeCol, vec3 horizonCol, vec3 zenithCol, vec3 viewDir, vec3 FOG_COLOR, float t, float rainFactor, bool end, bool underWater, bool nether) {
  ...
  if (end) {
    ...
    #endif
    if (!nether) {
      sky += getSunBloom(viewDir, FOG_COLOR);
    }
  }
