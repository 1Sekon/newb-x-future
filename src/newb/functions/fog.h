...
float nxfMistDensity(vec3 FOG_COLOR)
{
  return mix(mix(NXF_MIST_DENS_NOON, NXF_MIST_DENS_DAWN, duskD(FOG_COLOR)), NXF_MIST_DENS_NIGHT, nightD(FOG_COLOR));
}
...
float nlRenderFogFade(float relativeDist, vec3 FOG_COLOR, vec2 FOG_CONTROL) {
  ...
  // misty effect
  float density = nxfMistDensity(FOG_COLOR)*(19.0 - 18.0*FOG_COLOR.g);
